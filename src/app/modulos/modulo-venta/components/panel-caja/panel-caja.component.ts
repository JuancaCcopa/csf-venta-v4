...
export class PanelCajaComponent implements OnInit {
  ...
  onBuscarVenta() {
    ...
    this.goExisteVentaAnulada(numVentaBuscar).subscribe({
      ...
      next: data => {
        ...
        if (data > 0) {
          swal.fire(this.globalConstants.msgInfoSummary, `La venta tiene generada una anulación`, 'info');
          return
        }
...
debugger
    ...
    this.ventasCajaService.getVentaCabeceraPorCodVenta(codventa)
      .pipe(
        map((resp: any) => {
          ...
          const wCodcomprobante = resp.codcomprobante
          const wEstado = resp.estado
          const wTipoMov = resp.tipomovimiento

          if (wTipoMov == "DV" && wEstado == "G") {
            ...
            */
            ...
            swal.fire(this.globalConstants.msgInfoSummary, `Verifique la VENTA<br>Comprobante : ${wCodcomprobante}<br>Estado : ${wEstado}<br>Estado : ${wTipoMov}`, 'info');
            //Puede tener Comprobante o estar Anulado, estado=C o Mov=CA
          }
  ...
  goExisteVentaAnulada(codVenta: string) {
    return this.ventasCajaService.getExisteVentaAnuladaPorCodVenta(codVenta);
  }
  ...
  generarPagar() {
    ...
    */
    ...
    if (tipoComprobante == "") {
      swal.fire(this.globalConstants.msgInfoSummary, "SELECCIONE UN TIPO DE COMPROBANTE", 'warning')
      return;
    }
    ...
    //code 02
    if (comprobante != "") {
      swal.fire(this.globalConstants.msgInfoSummary, "Venta ya tiene Comprobante", 'warning')
      return;
    }

    //code 03
    if (cardCode == "") {
      swal.fire(this.globalConstants.msgInfoSummary, "El CardCode se encuentra vacio, escoja Para Quien es el comprobante...", 'warning')
      return;
    }

    //code 04
    if (cardCode.substring(0.1) == "M" || cardCode.substring(0.1) == "T") {
      swal.fire(this.globalConstants.msgInfoSummary, "No puede generar un comprobante a cardcode MED o TRC (Medico o Tercero). Consulte un cliente.", 'warning')
      return;
    }
    ...
    if (gratuito) {
      ...
      if (montoTotal > 0) {
        swal.fire(this.globalConstants.msgInfoSummary, "El comprobante GRATUITO no puede ser generado por un monto. Debe ser igual a cero", 'warning')
        return;
      }

      if (cbTipoAfectacionIgv.substring(0, 2) == "") {
        // If RTrim(Mid(Me.cboTipoAfectacionIgv.Text, 1, 2)) = "" Then 'AGARCIA.18/08/2016
        swal.fire(this.globalConstants.msgInfoSummary, "Elija el tipo de afectación de igv", 'warning')
        return;
      }
      ...
      //NO-Gratuito
      if (montoTotal <= 0) {
        swal.fire(this.globalConstants.msgInfoSummary, "El comprobante no puede ser generado por monto cero", 'question')
        return;
      }
  ...
  validarPagoBot() {
    ...
    if (codVenta === "") {
      swal.fire(this.globalConstants.msgInfoSummary, `Para los pagos de tipo farmacia, debe existir un codigo venta.`, 'warning');
      return;
    }
    ...
    this.ventasCajaService
      ...
      .subscribe(
        (resp: any) => {
          ...
          if (resp.est_pagado === "") {
            swal.fire(this.globalConstants.msgInfoSummary, `La venta ${codVenta} tiene una orden de pago pendiente.`, 'warning');
            return;
          }
          if (resp.est_pagado === "S") {
            swal.fire(this.globalConstants.msgInfoSummary, `Esta venta ya tiene un pago por medio de Bot o enlace de pago.`, 'warning');
            return;
          }
          if (resp.flg_pago_usado === "S") {
            swal.fire(this.globalConstants.msgInfoSummary, `Esta venta ya fue usado como forma de pago.`, 'warning');
            return;
          }
          ...
          if (montoTotal === 0) {
            swal.fire(this.globalConstants.msgInfoSummary, `No se puede generar una orden de pago con monto cero.`, 'warning');
            return;
          }
  ...
  obtenerLinkBot() {
    ...
    const { codVenta } = this.formularioCabecera.value;
    if (codVenta === "") {
      swal.fire(this.globalConstants.msgInfoSummary, `Para obtener el link bot debe existir un codigo venta.`, 'info');
      return;
    }

    this.ventasCajaService
      .getGetMdsynPagosConsulta("0", "0", "0", "", this.selectCodVenta, "6")
      .subscribe(
        (resp: any) => {

          if (resp.exito) {
            swal.fire(this.globalConstants.msgInfoSummary, `Copiar el link bot:<br><a href="${resp.mensaje}">${resp.mensaje}</a>`, 'info');
          } else {
            swal.fire(this.globalConstants.msgInfoSummary, resp.mensaje, 'info');
          }

        },
        (error) => {
          this.messageService.add({ key: 'toasVentaCaja', severity: 'error', summary: 'Mensaje', detail: ((typeof error.error) == 'string') ? error.error : error.error.resultadoDescripcion });
        }
      );
  }
    ...
    this.ventasCajaService
      ...
      .subscribe(
        (resp: any) => {
          ...
          if (resp.est_pagado == "") {
            swal.fire(this.globalConstants.msgInfoSummary, `Aún no se realizo el pago a la orden del bot.`, 'info');
            return
          } else {

            if (resp.flg_pago_usado == "S") {
              swal.fire(this.globalConstants.msgInfoSummary, `El pago de orden de la venta ya fue utilizado.`, 'info');
              return
            } else {
              ...
              this.listModeloPago.push(
                {
                  ...
                  codEntidad: "02",
                  nombreEntidad: resp.tip_tarjeta,
                  nroOperacion: resp.nro_operacion,
                  montoSoles: resp.cnt_monto_pago,
                  montoDolar: 0,
                  montoMn: resp.cnt_monto_pago,
                  codTerminal: '', // revisar en visual basic cuando tengas el ambiente de desarrollo para pago bot
