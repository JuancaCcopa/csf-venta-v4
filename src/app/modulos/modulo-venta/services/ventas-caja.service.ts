...
export class VentasCajaService {
  ...
  getExisteVentaAnuladaPorCodVenta(codventa: string) {
    let parametros = new HttpParams();
    parametros = parametros.append('codventa', codventa);
    return this.http.get<any>
      (`${environment.url_api_venta}VentaCaja/GetExisteVentaAnuladaPorCodVenta/`, { params: parametros });
  }
  ...
  setGenerarPagoBot(value: any) {
    const url = environment.url_api_venta + 'VentaCaja/GenerarPagoBot';
    const param: string = JSON.stringify(value);
    return this.http.post(url, param);
  }

  getObtnerPagoBot(idPagoBot: string,idmdsynReserva: string,idcorrelReserva: string,liquidacion: string,codVenta: string,orden: string) {
    let parametros = new HttpParams();
    parametros = parametros.append('ide_pagos_bot', idPagoBot);
    parametros = parametros.append('ide_mdsyn_reserva', idmdsynReserva);
    parametros = parametros.append('ide_correl_reserva', idcorrelReserva);
    ...
    parametros = parametros.append('orden', orden);
    return this.http.get<any>
      (`${environment.url_api_venta}VentaCaja/ObtenerPagoBot/`, { params: parametros,responseType: 'json' });
  }
